...
def fixed_point_convolve(a, v, mode='full'):
    ...
    output = output_type(output_int, output_fractional_bits, data_already_scaled=True)
...
def resize(fp_array, shape):
    ...
    '''
    return FixedPointArray(np.resize(fp_array.as_floating_point(), shape), fractional_bits=fp_array.fractional_bits)
...
def apply_along_axis(func1d, axis, arr, *args, **kwargs):
    ...
    return FixedPointArray(np.apply_along_axis(func1d, axis, arr.as_floating_point(), *args, **kwargs), fractional_bits)
...
def power(x1, x2, **kwargs):
    ...
    return FixedPointArray(np.power(x1.as_floating_point(), x2.as_floating_point(), **kwargs), x1.fractional_bits)

def abs(fd_array):
    return FixedPointArray(fd_array.as_floating_point(), fractional_bits=fd_array.fractional_bits)